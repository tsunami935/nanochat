...
from nanochat.tinyshakespeare_dataloader import tokenizing_distributed_data_loader_with_state, tokenizing_distributed_data_loader # modified nanochat.dataloader
from nanochat.common import compute_init, compute_cleanup, print0, DummyWandb, print_banner, get_base_dir, autodetect_device_type, get_peak_flops, COMPUTE_DTYPE, COMPUTE_DTYPE_REASON, is_ddp_initialized # checked: no change
from nanochat.tokenizer import get_tokenizer, get_token_bytes # checked: set NANOCHAT_BASE_DIR environment variable
from nanochat.checkpoint_manager import save_checkpoint, load_checkpoint # checked: no change
from nanochat.loss_eval import evaluate_bpb # checked: no change
from nanochat.engine import Engine # checked: no change
...
parser.add_argument("--dir", type=str, default=None, help="base directory")
# Logging
...
# Optimization
parser.add_argument("--device-batch-size", type=int, default=1, help="per-device batch size. good number to reduce to 16,8,4,... if you OOM on VRAM.")
...
if args.dir:
    os.environ["NANOCHAT_BASE_DIR"] = args.dir
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nanochat_shakespeare", name=args.run, config=user_config)
...
train_loader = tokenizing_distributed_data_loader_with_state(tokenizer, args.device_batch_size, args.max_seq_len, split="train", device=device, epoch=1)
...
x, y, epoch = next(train_loader) # kick off load of the very first batch of data
...
while True:
    ...
    if args.sample_every > 0 and master_process and (last_step or (step > 0 and step % args.sample_every == 0)):
        model.eval()
        prompts = [
            "LEAR:\nAnd my poor fool is hanged. No, no, no life?\nWhy should a dog, a horse, a rat have life,\nAnd thou no breath at all?", #King Lear - Act 5, scene 3, line 369
            "MACBETH:\nMy dearest love,\nDuncan comes here tonight.", # Macbeth - Act 1, scene 5, line 67
    ...
    for micro_step in range(grad_accum_steps):
        ...
        if scaler is not None:
            ...
            loss.backward()
        x, y, epoch = next(train_loader) # prefetch the next batch while the GPU is busy with forward/backward
    # step the optimizer
